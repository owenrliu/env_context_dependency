...
We join the monitoring and physical datasets and normalize all time series, as well as use linear interpolation to fill in the small amount of missing data (see `produce_final_data.R`):

```{r load data, message=FALSE,echo=F}
source("data/data preparation scripts/produce_final_data.R",local=knitr::knit_global())
```
```{r}
# icons of the focal species
...
purp_img <- magick::image_read_svg(here('icons', "purpleurchin-transparent.svg"))
red_img <- magick::image_read_svg(here('icons', "redurchin-transparent.svg"))
```
...
namekey <- bind_rows(namekey, tibble(long="NA",dataset="NA",short="const",plotting="Constant")) %>%
...
period.key <- tibble(year=rep(1980:2014,each=12),month=rep(1:12,35),period=c(rep(NA,8),rep(1,4),rep(2:69,each=6)))
...
westend.mean_algal_dens_plot <- westend.meandens %>%
  ...
  ggplot(aes(x=date_str,y=meandens,col=long))+
      ...
      scale_color_manual(values=cols8[1:4],
                         guide=guide_legend(title="",override.aes = list(size=0.5)))+
      guides(linetype="none")+
      theme(text = element_text(color="black",size=16),
            axis.title=element_text(size=10),
...
mei.pdo.npgo.ts.plot <-westend %>%
  ...
  scale_color_manual(values=cols8[1:3])+
...
sst.waves.ts.plot <-westend %>%
  ...
  geom_line(lwd=1.5)+
  geom_hline(yintercept=0,linetype=2,col="black")+
  scale_color_manual(values=cols8[5:6])+
...
We also want to look at prediction decay for each variable, which is one piece of evidence that a dynamic system is nonlinear. Using the best *E* identified in the previous step, we attempt to make predictions increasing far into the future, instead of just one period ahead. A nonlinear system should show decreasing predictive power with increasing prediction horizon[@Sugihara1994]. This phenomenon is a property of deterministic chaos and is analogous to the "butterfly effect", where in a nonlinear system, trajectories in state-space are expected to diverge over time. To examine this effect, we hold *E* constant, and proceed with simplex projection as before, but varying the prediction horizon, $t_p$ (i.e., how many steps ahead we try to predict).
...
for(i in 1:length(study_spp)) {
  spp <- study_spp[i]
  dat <- westend %>% select(matches(spp)) %>% as.data.frame()
  out <- purrr::map_df(1:10,function(x) simplex(as.numeric(dat[,1]),lib=as.matrix(westend.segs),silent=T,E=westend.bestE[spp],tp=x)) %>%
    mutate(spp=namekey$long[match(spp,namekey$short)]) %>% 
    mutate(across(where(is.list),as.double))
...
```{r ccm example}
westend.thin <- westend %>% dplyr::select(lam:sst)

tempE <- westend.bestE['purp']
temp <- suppressWarnings(ccm(westend.thin,lib=as.matrix(westend.segs),pred=as.matrix(westend.segs),E=tempE,lib_column= 'mac',target_column = 'purp',lib_sizes = c(10,25,50,75,100,125,150,200,300,400,500),num_samples=150,replace=T,silent=T,RNGseed = 41389))

mac_xmap_purp <- temp %>%
  ungroup()%>%
  ggplot(aes(LibSize,`mac:purp`))+
...
In this example, we are asking whether there is evidence that *S. purpuratus* drives *Macrocystis* dynamics, and we evaluate that using library vectors from the *Macrocystis* time series to try to predict densities of *S. purpuratus*. In this case, the answer is yes: there is evidence that *S. purpuratus* drives *Macrocystis*, because the above signal is both significantly greater than zero, and is convergent.

Now we apply the above process to all possible species interactions and environment-species forcings.

```{r}
ccm_to_test <- tidyr::crossing(x=c(study_spp,phys.vars),y=c(study_spp,phys.vars)) %>% 
  mutate(Etarg=westend.bestE[y]) %>% 
  filter(x!=y) %>% 
  filter(!(x%in%phys.vars))

two_way_ccm <- function(x,y,Etarg){
  pr() # for tracking progress
  ccm1 <- suppressWarnings(ccm(westend.thin,lib=as.matrix(westend.segs),pred=as.matrix(westend.segs),E=Etarg,lib_column= x,target_column = y,lib_sizes = c(10,500),num_samples=100,replace=T,stats_only=F,silent=T,RNGseed = 41389))
  out <- bind_rows(ccm1$CCM1_PredictStat,ccm1$CCM2_PredictStat)
  toc()
  return(out)
}

# test purp
# testpurp <- two_way_ccm("purp","mei",9)
```
...
with_progress({
  pr <- progressor(steps=nrow(ccm_to_test))
  westend.ccm<- furrr::future_pmap_dfr(ccm_to_test,two_way_ccm)
})

westend.ccmmeans <- westend.ccm %>% 
  group_by(lib,target,E,LibSize) %>% 
  mutate(ispos=rho>0) %>% 
  # first check- are more than 95% of sample rhos at a library size of 500 greater than zero?
  summarise(rhomean=mean(rho,na.rm=T),rhosd=sd(rho,na.rm=T),p1=1-(sum(ispos)/n())) %>% 
  ungroup() %>% 
  group_by(lib,target,E) %>% 
  mutate(dif=rhomean[LibSize==500]-rhomean[LibSize==10]) %>% 
  ungroup() %>% 
  mutate(rholow=rhomean-rhosd,rhohigh=rhomean+rhosd) %>% 
  mutate(Etarg=westend.bestE[target]) %>% 
  filter(Etarg==E) %>% 
  dplyr::select(-Etarg)

p1 <- westend.ccmmeans %>% 
  filter(LibSize==500) %>% 
  mutate(p1check=ifelse(p1<=0.05,1,0))
p2 <- westend.ccm %>%
  group_split(lib,target) %>% 
  purrr::map_df(function(d){
     p=t.test(d$rho[d$LibSize==10],d$rho[d$LibSize==500])$p.value
     d %>% distinct(lib,target) %>% mutate(p2=p)
  }) %>% 
  left_join(westend.ccmmeans) %>%
  mutate(p2=ifelse(dif<=0.05,1,p2)) %>% 
  mutate(p2check=ifelse(p2<=0.05,1,0)) %>% 
  dplyr::select(lib,target,dif,p2,p2check) %>% 
  distinct() %>% 
  drop_na()
```
...
make_100_surrogates <- function(spp){
  westend %>% 
    dplyr::select(site,all_of(spp))%>% 
    rename(orig=2) %>% 
    # make surrogate time series using rEDM::make_surrogate_data, while making sure to maintain seams in the original data
    group_by(site) %>%
    nest() %>% 
    mutate(surr=purrr::map(data,function(df){make_surrogate_data(df$orig,method="seasonal",num_surr=100,T_period=2)})) %>% 
    ungroup() %>% 
    dplyr::select(surr) %>% 
    .[[1]] %>% 
    do.call(rbind,.)
...
surrogates <- purrr::map(c(study_spp,phys.vars),make_100_surrogates) %>% set_names(c(study_spp,phys.vars))

test_p3 <- function(libspp,tarspp){
  pr() # for tracking progress
  orig <- westend[[libspp]]
  surr <- surrogates[[tarspp]]
  surr_rhos <- numeric()
  for(k in 1:ncol(surr)){
    tempccm <- ccm(cbind(orig, surr[,k]), E = westend.bestE[[tarspp]],lib=as.matrix(westend.segs),pred=as.matrix(westend.segs),lib_column = 1, target_column = 2, lib_sizes = c(500),num_samples=10,replace=F,silent=T,RNGseed = 41389,stats_only = F)$CCM1_PredictStat
    surr_rhos[k] <- tempccm %>% group_by(LibSize) %>% summarise(rhomean=mean(rho,na.rm=T)) %>% ungroup() %>% pull(rhomean)
  }
  return(surr_rhos)
}
...
with_progress({
  pr <- progressor(steps=nrow(ccm_to_test))
  p3_list <- future_map2(ccm_to_test$x,ccm_to_test$y,test_p3)
})

p3_df <- ccm_to_test %>% 
  mutate(p3rho=p3_list) %>% 
  left_join(p1,by=c("x"="lib","y"="target")) %>% 
  mutate(p3check=purrr::map2(p3rho,rhomean, function(i,j) sum(i>j))) %>% 
  # is the original rho greater than 95% of the surrogate rhos?
  # if fewer than 6 (out of 100) estimated rhos are greater than the original ccm rhomean, test is passed
  mutate(p3check=ifelse(p3check<6,1,0)) %>% 
  dplyr::select(lib=x,target=y,p3check)

all_checks <- p1 %>% dplyr::select(lib,target,E,rhomean,p1check) %>% left_join(p2) %>% left_join(p3_df) %>% 
  # does the CCM pass all checks?
  mutate(is_signif=ifelse((p1check+p2check+p3check)==3,1,0))
future::plan(sequential)
```

```{r}
# plot
westend.xmap_rast <- all_checks %>% 
  rename(predictor=target,predictee=lib) %>% 
  ...
  mutate(rho=ifelse(is_signif==0,NA,rhomean))

westend.xmap_rast <- westend.xmap_rast %>% 
  bind_rows(tibble(force.name=unique(namekey$plotting)[1:6],pred.name=unique(namekey$plotting)[1:6]))
...
westend.xmap_all_plot <- ggplot(westend.xmap_rast,aes(x=force.name,y=pred.name,fill=rho)) +
  ...
  geom_text(aes(label=round(rho,3)),size=3)+
...
From this plot, we can make a few observations. The five physical forcing variables are represented in the first five columns, followed by the algae and urchin species. Many physical variables show causal relationships with kelp forest species, especially the North Pacific Gyre Oscillation and Pacific Decadal Oscillation, which show causal links to all of the biological variables. The NPGO is cross-mapped well by the purple urchin *Strongylocentrotus purpuratus*, while the NPGO, PDO and MEI indices all show a strong influence on adult *Macrocystis* dynamics. Links between SST and physical disturbance (i.e., significant wave height) and the focal study species are non-significant or small.

Additionally, adult *Macrocystis* itself seems to be an important driver of the dynamics of all the other species in the tropic web, supporting the decades of research showing the importance of giant kelp as a foundation species [@Graham2007c;@Dayton1985], but further suggesting that its dynamics fundamentally drive the dynamics of other species. Interestingly, while juvenile and adult *Macrocystis* and *Pterygophora* seem to be strongly driven by both environmental and urchin dynamics, *Laminaria* seems to be driven less by the oceanographic variables, and more by the dynamics of its competitors (i.e., *Macrocystis* and *Pterygophora*) and consumers (i.e., urchins).

Both sea urchin species are causally linked to *Macrocystis* density and to one another. This analysis alone, however, does not yet tell us whether the interaction between *S. purpuratus* and *M. franciscanus* represents apparent mutualism, apparent competition, or a mixture of both at different times. To further investigate the direction and magnitude of the species interactions themselves, we next build multivariate EDM models that explicitly measure dynamic species interaction strengths (and directions).
...
connect.mat <- tidyr::crossing(predictee=c(study_spp,phys.vars),predictor=c(study_spp,phys.vars)) %>% 
  left_join(westend.xmap_rast %>% dplyr::select(predictee,predictor,rho)) %>% 
  mutate(connected=ifelse(is.na(rho),0,1)) %>% 
  dplyr::select(predictee,predictor,connected) %>% 
  pivot_wider(names_from = "predictee",values_from = "connected") %>% 
# reorder such that similar variables are adjacent, for graphing purposes
  slice(c(1,2,6,11,7,8,3,4,5,9,10)) %>% 
  dplyr::select(-1) %>% 
  as.matrix()
# reorder such that similar variables are adjacent, for graphing purposes
connect.mat <- connect.mat[,c(1,2,6,11,7,8,3,4,5,9,10)]

# edges (arrows) weighted based on CCM rho value
edge.weight.mat <- tidyr::crossing(predictee=c(study_spp,phys.vars),predictor=c(study_spp,phys.vars)) %>% 
  left_join(westend.xmap_rast %>% dplyr::select(predictee,predictor,rho)) %>% 
  dplyr::select(predictee,predictor,rho) %>% 
  pivot_wider(names_from = "predictee",values_from = "rho") %>% 
# reorder such that similar variables are adjacent, for graphing purposes
  slice(c(1,2,6,11,7,8,3,4,5,9,10)) %>% 
  dplyr::select(-1) %>% 
  as.matrix()
edge.weight.mat[is.na(edge.weight.mat)]<-0
edge.weight.mat <- edge.weight.mat[,c(1,2,6,11,7,8,3,4,5,9,10)]

names <- map_chr(colnames(connect.mat), function(x) as.character(namekey$plotting[match(x,namekey$short)]))
names[4] <- "M. pyr\n(j)"
names [c(7:11)] <- c("MEI","NPGO","PDO","SST","SWH")
...
vert.cols <- c(rep("#238B45",4),rep("#BEBADA",2),rep("#8DD3C7",5))
...
plot(g2,asp=0.8, vertex.label.color="black",vertex.label.family="HersheySans", vertex.label.cex=0.7, edge.arrow.size=0.8,edge.arrow.width=0.8,vertex.label.font=2, vertex.shape="circle", edge.curved=curves,margin=0,vertex.size=20,rescale=T,edge.lty=1,edge.color=edge.cols,layout=layout_in_circle,vertex.color=vert.cols,vertex.frame.color="black",main="")
# dev.off()
```

```{r}
net_img <- magick::image_read_pdf("C:/Users/Owen.Liu/Documents/github/EDM_SNI/plots/ccm_network.pdf")

network_fig_icons<- ggdraw()+
  draw_image(net_img)+
  draw_image(red_img,x=0.15,y=0.53,width=0.15,height=0.15,clip = 'on')+
  draw_image(purp_img,x=0.23,y=0.7,width=0.22,height=0.22,clip = 'on')+
  draw_image(ymac_img,x=0.38,y=0.79,width=0.2,height=0.2,clip = 'on')+
  draw_image(pter_img,x=0.53,y=0.77,width=0.2,height=0.2,clip = 'on')+
  draw_image(mac_img,x=0.65,y=0.67,width=0.2,height=0.2,clip = 'on')+
  draw_image(lam_img,x=0.7,y=0.45,width=0.2,height=0.2,clip = 'on')

# ggsave("C:/Users/Owen.Liu/Documents/github/EDM_SNI/plots/ccm_network_icons.pdf",plot=network_fig_icons,h=4,w=6)
```


In this network, each node represents a variable. The maximum number of outgoing interactions is 5 (6 total biological variables minus the variable itself), and incoming is 10 (5 biological and 5 physical variables). Algae species are in dark green, while the two urchin species are in purple and the physical drivers are in blue-green. Again, we see here as above that young *Macrocystis* (denoted M. pyr (j))is forced by all other variables in the system except SST, and in turn shows a causal forcing on the 5 other biological variables, with varying cross-mapping "strength" (correlation, edge weight in the network graph). Again we can see that while the web is not entirely saturated, there are many unidirectional forcings, and a prevalence of bidirectional causation as well (A forces B *and* B forces A). The adult *Macrocystis* is causally connected to all variables in the analysis except SST.
...
# EDIT 04.25.2022: block_lnlp needs a fix in the summarizing of the s-map model
source("block_lnlp_fix.R")

smap_multi <- function(sitedat,libsegs,predsegs=libsegs,species,causalvars,...) {
  
  full.mod <- suppressWarnings(block_lnlp(sitedat,lib=as.matrix(libsegs),pred=as.matrix(predsegs),columns=causalvars,target_column = species,theta = c(0, 1e-04, 3e-04, 0.001,0.003, 0.01, 0.1, 0.5, 1, 2, 4, 6,8), num_neighbors=0,method="s-map",silent=T))
  ...
  # t <- best.mod$smap_coefficients[[1]]$Index
  
  coeff.names <- c('time',causalvars,"const")
  ...
  coeffs <- coeffs %>% mutate(period=c(1,rep(1:63,10)))
  ...
  outlist <- list(model=best.mod,coeffs=coeffs,
                  # coeffs.variances=coeffs.variances,
                  coeffs.long= coeffs.long,plot=int.box)
  return(outlist)
}
```
...
westend.multi.mods <- purrr::map(list("mac","purp","lam","ymac","red","pter"),function(x) {
  smap_multi(sitedat=westend.thin,libsegs=westend.segs,species=x,causalvars=westend.causal.vars[[x]])
})
...
It is clear from the boxplots that fitted intra- and interspecific interactions display evidence of positive, negative, and neutral interactions. Over the range of conditions in the time series, each species variable with the exception of *Macrocystis* has positive intraspecific effects; that is, each species’ density has a positive estimated effect on itself, a combination of survivorship and new recruitment. Relative to the magnitude of other interactions, this intraspecific effect is strongest in the two urchin species.

The interactions of the urchin and algal species show evidence of herbivory (purple interactions in the plot). Both red and purple urchin density have predominantly negative effect on adult and juvenile *Macrocystis* and *Pterygophora*, effects that were consistent with our hypotheses. The exception is a predominantly positive effect of the purple urchin *S purpuratus* on *Laminaria*. The converse effects of algal density on urchin density (green boxes) are generally small, except again for the effect of *Laminaria*, which is consistently positive.

Few strong and consistent competitive interactions (red and orange boxes for urchin and algal competition, respectively) are observed in model estimates. Estimates of the interaction between the two urchin species are near zero. The effect of red on purple urchins is small relative to other forcing factors in the urchin models, and is occasionally positive (apparent mutualism). Among the algal species, the clearest negative competitive effect is that of *Pterygophora* on young *Macrocystis*. Other interactions are not always negative (competitive), and contrary to expectations, positive values were estimated for most algal interactions. Perhaps most surprisingly, *Macrocystis* does not display a consistent negative effect on *Pterygophora*, contrary to our hypothesis of *Macrocystis* competitive dominance, and instead has a positive estimated interaction under most conditions [@Dayton1999].
...
Physical forcing variables affect the dynamics of all species in the study, to varying degrees. According to CCM analysis, no species were driven directly by SST, instead responding to the manifestations of the PDO, NPGO, and MEI. The positive mean effect of the NPGO on *Macrocystis* is consistent with our expectations [@Bell2015]. However, the PDO index has a positive effect on *Macrocystis*, contrary to expectation because in general, positive values of the PDO are associated with warmer SST and nutrient-poor conditions in the northeast Pacific. *Macrocystis* recruits, on the other hand, are positively affected by the NPGO  as well, by predominantly negatively affected by the PDO, SWH, and MEI. Together, the effects of physical forcing on *Macrocystis* suggest that the high phases of the NPGO may be favorable for both recruitment and growth of the giant kelp, while conditions associated with the high phase of the PDO may indicate favorable conditions for survivorship or growth, but poor conditions for the recruitment of new sporophytes.

The physical variables are more variable drivers of *Macrocystis* than *Pterygophora* dynamics. All the algal species were predominantly positively associated with greater values of the NPGO and negatively with the MEI.

For the purple and red urchins, SWH and NPGO have on average negative but variable effects, while the effects of the PDO and MEI are near zero or slightly positive.
...
interaction_distribution_by_type <- all.mod.coeffs.long %>%
  ...
  geom_density(alpha=0.8,color=NA,fill='gray50')+
...
ymac_ints_plot <- ggplot(ymac_ints_ts,aes(date_str,meanval,col=plotting))+
  geom_line(size=1.5)+
  scale_color_manual(values=cols8[c(2,3,5,6)])+
  annotate("rect",xmin=as_date("2000-01-01"),xmax=as_date("2002-01-01"),ymin=-1.9,ymax=1.4,fill=NA,col='black')+
  annotate("rect",xmin=as_date("2004-06-01"),xmax=as_date("2006-06-01"),ymin=-1.9,ymax=1.4,fill=NA,col='black')+
  geom_hline(yintercept=0,linetype=2)+
...
npgo_high_ymac <- ymac_ints_ts %>% 
  ...
  geom_col()+
  geom_hline(yintercept=0)+
  scale_fill_manual(values=cols8[c(2,3,5,6)],guide="none")+
  scale_y_continuous(limits=c(-2,0.7))+
...
connect.mat.algae <- tidyr::crossing(predictee=c('lam','mac','pter','ymac'),predictor=c('lam','mac','pter','ymac')) %>% 
  left_join(westend.xmap_rast %>% dplyr::select(predictee,predictor,rho)) %>% 
  mutate(connected=ifelse(is.na(rho),0,1)) %>% 
  dplyr::select(predictee,predictor,connected) %>% 
  pivot_wider(names_from = "predictee",values_from = "connected") %>% 
...
# edges (arrows) weighted based on CCM rho value
edge.weight.mat.algae <- tidyr::crossing(predictee=c('lam','mac','pter','ymac'),predictor=c('lam','mac','pter','ymac')) %>% 
  left_join(westend.xmap_rast %>% dplyr::select(predictee,predictor,rho)) %>% 
  dplyr::select(predictee,predictor,rho) %>% 
  pivot_wider(names_from = "predictee",values_from = "rho") %>% 
  dplyr::select(-1) %>% 
...
npgo_sgh_plot <- npgo_sgh %>% 
  ...
  geom_smooth(se=F)+
  scale_color_manual(values=cols8[c(1,2,3,4,5,6)])+
  scale_fill_manual(values=cols8[c(1,2,3,4,5,6)])+
  # scale_color_npg()+
  # scale_fill_npg()+
  geom_hline(yintercept = 0,linetype=2)+
  geom_vline(xintercept = 0,linetype=2)+
...
grDevices::cairo_pdf("C:/Users/Owen.Liu/Documents/github/EDM_SNI/plots/npgo_sgh_fig.pdf",
                     width=unit(6, "in"),height=unit(4,"in"))
npgo_sgh_fig
dev.off()
# ggsave("C:/Users/Owen.Liu/Documents/github/EDM_SNI/plots/npgo_sgh_fig.pdf",plot=npgo_sgh_fig,h=4,w=6)
...
npgo_sgh_ymac_plot <- npgo_sgh_ymac %>% 
  ...
  geom_smooth(se=F)+
  scale_color_manual(values=cols8[c(2,3,5,6)])+
  scale_fill_manual(values=cols8[c(2,3,5,6)])+
  geom_hline(yintercept = 0,linetype=2)+
  geom_vline(xintercept = 0,linetype=2)+
...
# ggsave("C:/Users/Owen.Liu/Documents/github/EDM_SNI/plots/npgo_sgh_fig5.pdf",plot=fig5,h=8,w=6)
grDevices::cairo_pdf("C:/Users/Owen.Liu/Documents/github/EDM_SNI/plots/npgo_sgh_fig5.pdf",
...
back_select_multivariate_model <- function(sitedat=westend,libsegs=westend.segs,predsegs=libsegs,species,causalvars,...) {
  ptm <- proc.time()
  nsteps <- min(length(causalvars),7)
  ...
  sitedat_mod <- sitedat %>% bind_cols(lagsblock) %>% dplyr::select(-site,-period)
  
  # Run full model, including all CCM-validated causal variables
  full.mod <- suppressWarnings(block_lnlp(sitedat_mod,lib=as.matrix(libsegs),pred=as.matrix(libsegs),columns=causalvars,target_column = species,theta = c(0, 1e-04, 3e-04, 0.001,0.003, 0.01, 0.1, 0.5, 1, 2, 4, 6,8), num_neighbors=0,method="s-map",silent=T))
  ...
  for(i in 2:nsteps) {
    ...
    for(j in 1:length(poss_vars)) {
      ...
      temp_sitedat <- sitedat
      temp_libsegs <- libsegs
      if(bestEdiff>0) {
        temp_vars <- c(temp_vars,paste0('lag',1:bestEdiff))
        temp_sitedat <- sitedat %>% bind_cols(lagsblock) %>% dplyr::select(c(site,temp_vars,species)) %>% drop_na()
        temp_libsegs <- temp_sitedat %>%
          ungroup() %>%
          mutate(ind = row_number()) %>% 
          group_by(site) %>%
          summarise(first=first(ind),last=last(ind)) %>% 
      ...
      temp_sitedat <- temp_sitedat %>% dplyr::select(-site)
      
      temp <- suppressWarnings(block_lnlp(temp_sitedat,lib=as.matrix(temp_libsegs),pred=as.matrix(temp_libsegs),columns=temp_vars,target_column = species,theta = c(0, 1e-04, 3e-04, 0.001,0.003, 0.01, 0.1, 0.5, 1, 2, 4, 6,8), num_neighbors=0,method="s-map",silent=T))
  ...
  selected.model <- suppressWarnings(smap_multi(sitedat=westend.thin,libsegs=westend.segs,species=species,causalvars=selected.vars))
...
backwards_selection <- furrr::future_map(list("mac","purp","lam","ymac","red","pter"),function(x) {
