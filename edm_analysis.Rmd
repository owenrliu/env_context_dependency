...
plot_jacobian_comparison <- function(model,eff1,eff2){
  ...
  minmax <- c(min(dat[,4]-0.1),max(dat[,4]+0.1),min(dat[,3]-0.1),max(dat[,3]+0.1))
  ...
  dat %>% 
    ...
    annotate("text",x = minmax[1]+0.2,y=0.1,label="Facilitation",size=4,family="Times New Roman")+
    annotate("text",x = minmax[1]+0.2,y=-0.1,label="Competition",size=4,family="Times New Roman")+
    geom_hline(yintercept=0,linetype=2)+
    geom_vline(xintercept=0,linetype=2)+
...
# NPGO, Macrocystis, and Laminaria
plot_jacobian_comparison('lam','mac','npgo')

# NPGO, Pterygophora, and Laminaria
plot_jacobian_comparison('lam','pter','npgo')

# NPGO, Pterygophora, and Young Macrocystis
plot_jacobian_comparison('ymac','pter','npgo')

# Herbivory, Pterygophora, and Laminaria
plot_jacobian_comparison('lam','pter','purp')
```
...
In the figure below, each bar represents the percentage of each species’ effects that are significantly positive or negative (effects not significantly different than zero not shown). Note varying scale on y-axis.
