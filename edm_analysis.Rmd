...
We can observe some coherence between some estimated species interactions and environmental variables. For example, the North Pacific Gyre Oscillation index is one environmental variable driving all of the algal species in our analysis. The figure below shows how the NPGO varies, as well as time series of the estimated effects of *Laminaria* and *Pterygophora* on *Macrocystis* recruits. Immediately, the time-varying effects of the two understory species on the recruitment of *Macrocystis* are evident, as well as their apparent (if complex) coherence with fluctuations in the NPGO index.

```{r ymac ints ts,fig.width=8,fig.height=4}
ymac_ints_ts <- all.mod.coeffs.long %>% 
  filter(mod=='ymac',type%in% c('instraspecies','algal competition'),effect!='ymac') %>% 
