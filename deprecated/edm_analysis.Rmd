...
temp <- suppressWarnings(ccm(westend %>% dplyr::select(lam:sst),lib=as.matrix(westend.segs),pred=as.matrix(westend.segs),E=tempE,lib_column= 'mac',target_column = 'purp',lib_sizes = c(10,25,50,75,100,125,150,200,300,400,500),num_samples=150,replace=T,silent=T,RNGseed = 41389))
...
two_way_ccm <- function(x,y,Etarg){
  ...
  ccm1 <- suppressWarnings(ccm(westend %>% dplyr::select(lam:sst),lib=as.matrix(westend.segs),pred=as.matrix(westend.segs),E=Etarg,lib_column= x,target_column = y,lib_sizes = c(10,500),num_samples=100,replace=T,stats_only=F,silent=T,RNGseed = 41389))
...
for(i in 1:n_row) {
  for(j in 1:n_col) {
    if(i != j) {
      ...
      temp <- suppressWarnings(ccm(westend %>% dplyr::select(lam:sst),lib=as.matrix(westend.segs),pred=as.matrix(westend.segs),E=tempE,lib_column= i,target_column = j,lib_sizes = c(10,500),num_samples=100,replace=T,stats_only=F,silent=T,RNGseed = 41389))
     ...
     # mean rho at library size 500
      ...
      surr_ts <- westend %>% select(site,2+j) %>% rename(orig=2) %>% 
        ...
        nest() %>% 
        mutate(surr=purrr::map(data,function(df){make_surrogate_data(df$splined,num_surr=100,T_period=2)})) %>% 
        ungroup() %>% 
        select(surr) %>% 
        .[[1]] %>% 
        do.call(rbind,.)
...
network_fig_icons<- ggdraw()+
  draw_image(net_img)+
  draw_image(ymac_img,x=0.15,y=0.5,width=0.2,height=0.2,clip = 'on')+
  draw_image(red_img,x=0.23,y=0.7,width=0.15,height=0.15,clip = 'on')+
  draw_image(purp_img,x=0.38,y=0.77,width=0.2,height=0.2,clip = 'on')+
